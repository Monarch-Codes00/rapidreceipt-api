...
public class RefreshTokenService {
    ...
    public RefreshToken createRefreshToken(Long userId) {
        ...
        // Enforce 1 active session by deleting any existing refresh tokens for this user
    ...
    public RefreshToken verifyExpiration(RefreshToken token) {
        if (token.getExpiryDate().compareTo(Instant.now()) < 0) {
            ...
            throw new ApiException("Refresh token was expired. Please make a new signin request", HttpStatus.UNAUTHORIZED);
        }
        return token;
    }
}
